...
import { MCQAPI } from "../../api/mcqs"; // make sure this points to your API service
...
export default function MyMCQsPage() {
  ...
  useEffect(() => {
    const loadMCQs = async () => {
      try {
        const data = await MCQAPI.getMCQSets();
        console.log(data)
        setMcqs(data);
      } catch (err) {
  ...
  const confirmDelete = async () => {
    try {
      await MCQAPI.deleteMCQSet(mcqToDelete);
          ...
          {mcqs.length > 0 ? (
            mcqs.map((mcq) => {
              ...
              const displayTitle = isExpanded
                ? mcq.title
                : truncateText(mcq.title, wordsPerLine);
